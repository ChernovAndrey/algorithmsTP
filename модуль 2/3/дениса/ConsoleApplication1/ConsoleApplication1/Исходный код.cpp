...
void HeapSort(vector<T> &arr) {
	...
	//«абираем максимальный (0) элемент дерева в i-ю позицию
	//ѕеремещаем новый 0 элемент на правильную позицию в дереве
	for (i = l - 1; i > 0; i--) {
		swap(arr[i], arr[0]);
		ShiftDown(arr, 0, i);
	}
...
int GetResult(vector<T> &arr) {
	...
	for (int i = 1; i < n; ++i) { // идем по промежуткам времени посетителей
		if (adv1 < arr[i].start){  // если реклама предыдущего посетител€ не попадает в период посещени€ текущему
			if (adv2 == arr[i].finish){ // если втора€ реклама уже стоит в конце, то первую ставим в предконец, если нет, то в конец
				adv1 = arr[i].finish - 1;
			}
			else {
				adv1 = arr[i].finish;
		...
		if (adv2 < arr[i].start){ // если реклама предыдущего посетител€ не попадает в период посещени€ текущему
